...
const Toast = () => {
    ...
    const { store, hideToast } = useContext(StoreContext)
    ...
    return (
        <div id="toast" className={`${store.toast.isToast ? 'translate-x-0' : 'translate-x-96'} z-[10000] transition relative flex items-center w-full max-w-xs p-4 mb-4 text-gray-700 bg-white rounded-lg shadow-md dark:text-gray-400 dark:bg-gray-800" role="alert`}>
            <div className={`inline-flex items-center justify-center flex-shrink-0 w-8 h-8 ${store.toast.isSuccess ? 'text-green-700 bg-green-100 dark:bg-green-800 dark:text-green-200' : 'text-red-700 bg-red-100 dark:bg-red-800 dark:text-red-200'} rounded-lg `}>
            ...
            <div className="ml-3 text-sm font-normal">{store.toast.text}</div>
