...
import CategoriesForm from "../../shared/forms/CategoriesForm"
...
const CategoriesTool = () => {
    ...
    const Content = () => {
      return (
        <div className="px-6 pb-4 space-y-6 lg:px-8 sm:pb-6 xl:pb-8">
          <h3 className="text-xl font-medium text-gray-900 dark:text-white">Add Category</h3>
          <CategoriesForm onSubmit={handleAddSubmit} />
        </div>
      )
    }
    showModal(Content)
  ...
  const modalEdit = (index) => {
    const initStates = {
      id: store.appData.categories[index].id,
      name: store.appData.categories[index].name,
      origin: store.appData.categories[index].origin,
    }
    ...
    const Content = () => {
      return (
        <div className="px-6 pb-4 space-y-6 lg:px-8 sm:pb-6 xl:pb-8">
          ...
          <CategoriesForm onSubmit={handleEditSubmit} initStates={initStates} />
        </div>
      )
    }
  ...
  const handleDelete = async (index) => {
    setLoading(true)
    const bid = store.appData.categories[index].id
    /* Send data to API to register a new user */
    const config = {
      method: 'delete',
      ...
      {store.loading
        ...
        : (
          <div className='grid place-items-center'>
            <h1 className='text-left text-xl font-medium p-6 text-gray-700'>Categories Data</h1>
                ...
                <div className="p-4 flex">
                  ...
                  <div className="relative mt-1">
                    ...
                    <input
                      onChange={(e) => {
                        if (e.target.value !== '') {
                          setSearchResults(store.appData.categories.filter(user => user.first_name.toLowerCase().includes(e.target.value)))
                        } else {
                          setSearchResults(store.appData.categories)
                        }
                      }}
